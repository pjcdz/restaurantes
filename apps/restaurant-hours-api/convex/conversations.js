...
const sessionValidator = v.object({
    ...
    createdAt: v.number(),
    updatedAt: v.number(),
    status: v.union(v.literal("active"), v.literal("handed_off"), v.literal("paused"))
});
...
const pedidoValidator = v.object({
    ...
    total: v.number(),
    estado: v.union(v.literal("completo"), v.literal("error_producto"), v.literal("incompleto")),
    createdAt: v.number(),
    updatedAt: v.number()
});
...
export const listCatalogItemsForAdmin = query({
    ...
    handler: async (ctx) => {
        ...
        return Array.from(productsByName.values()).sort((left, right) => left.item.localeCompare(right.item, "es"));
    }
});
...
export const getSessionStatus = query({
    ...
    returns: v.union(v.literal("active"), v.literal("handed_off"), v.literal("paused"), v.null()),
    handler: async (ctx, args) => {
